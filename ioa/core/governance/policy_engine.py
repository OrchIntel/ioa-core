...
class PolicyEngine:
    ...
    def __init__(self):
        ...
        # Security hardening: optional service authentication gating
        # Env: IOA_REQUIRE_AUTH (default true), IOA_AUTH_TOKEN (required if gating enabled)
        require_auth_env = os.getenv("IOA_REQUIRE_AUTH", "true").lower()
        self._require_auth: bool = require_auth_env in ("1", "true", "yes")
        self._auth_token: Optional[str] = os.getenv("IOA_AUTH_TOKEN")
    ...
    def validate_against_laws(self, action_ctx: ActionContext) -> ValidationResult:
        ...
        # Enforce service authentication if enabled
        if self._require_auth:
            provided = None
            try:
                provided = action_ctx.metadata.get("auth_token")  # type: ignore[assignment]
            except Exception:
                provided = None
            if not self._auth_token or provided != self._auth_token:
                logger.error("PolicyEngine authentication failed: missing/invalid auth token")
                raise PermissionError("PolicyEngine authentication failed")
    ...
    def _check_governance_precedence(self, action_ctx: ActionContext) -> Dict[str, Any]:
        ...
        # Check if action is trying to bypass governance
        if action_ctx.metadata.get("bypass_governance", False):
            return {
                "violation": True,
                "details": "Actions cannot bypass governance policies"
            }
        
        # Check if action is trying to modify governance rules
        if action_ctx.action_type in ["policy_modification", "governance_override"]:
            return {
                "violation": True,
                "details": "Governance policies cannot be modified during runtime"
            }
        
        # Check if action has governance override flag
        if action_ctx.metadata.get("governance_override", False):
            return {
                "violation": True,
                "details": "Governance override not allowed - policies take precedence"
            }
    ...
    def _check_auditability(self, action_ctx: ActionContext) -> Dict[str, Any]:
        ...
        # Check if action has required audit fields
        if not action_ctx.action_id:
            return {
                "violation": True,
                "details": "Action must have a unique action_id for audit trail"
            }
        
        # Check if action has timestamp
        if not action_ctx.timestamp:
            return {
                "violation": True,
                "details": "Action must have a timestamp for audit trail"
            }
        
        # Check if action has actor identification
        if not action_ctx.actor_id:
            return {
                "violation": True,
                "details": "Action must have an actor_id for audit trail"
            }
        
        # Check if sensitive actions have proper audit context
        if action_ctx.data_classification in ["personal", "sensitive", "restricted"]:
            if not action_ctx.metadata.get("audit_context"):
                return {
                    "violation": True,
                    "details": "Sensitive data actions must include audit context"
                }
    ...
    def _check_immutable_governance(self, action_ctx: ActionContext) -> Dict[str, Any]:
        ...
        if action_ctx.action_type in ["reflex_action", "auto_execution"]:
            ...
            # Check if reflex action is trying to modify governance
            if action_ctx.metadata.get("modify_governance", False):
                return {
                    "violation": True,
                    "details": "Reflex actions cannot modify governance rules"
                }
            
            # Check if auto-execution is within time bounds
            if action_ctx.action_type == "auto_execution":
                max_auto_duration = action_ctx.metadata.get("max_duration_seconds", 300)  # 5 minutes default
                if max_auto_duration > 3600:  # 1 hour max
                    return {
                        "violation": True,
                        "details": "Auto-execution duration exceeds maximum allowed (1 hour)"
                    }
        
        # Check if action is trying to modify system laws
        if action_ctx.action_type in ["system_law_modification", "governance_manifest_update"]:
            return {
                "violation": True,
                "details": "System laws are immutable and cannot be modified"
            }
    ...
    def _check_fairness(self, action_ctx: ActionContext) -> Dict[str, Any]:
        ...
        # Only check fairness for actions that produce outputs
        if action_ctx.action_type not in ["llm_generation", "decision_making", "content_creation", "user_interaction"]:
            return {"violation": False}
        
        # Check for explicit bias indicators in metadata
        if action_ctx.metadata.get("bias_indicators"):
            bias_indicators = action_ctx.metadata["bias_indicators"]
            if any(indicator in bias_indicators for indicator in ["gender", "race", "age", "religion", "disability"]):
                return {
                    "violation": True,
                    "details": f"Action contains potential bias indicators: {bias_indicators}"
                }
        
        # Check for demographic targeting in metadata
        if action_ctx.metadata.get("demographic_targeting"):
            return {
                "violation": True,
                "details": "Demographic targeting is not allowed for fairness compliance"
            }
        
        # Check for discriminatory language patterns
        if action_ctx.metadata.get("content_text"):
        ...
        if action_ctx.action_type == "decision_making":
            if not action_ctx.metadata.get("equal_opportunity_check", False):
                return {
                    "violation": True,
                    "details": "Decision-making actions must include equal opportunity assessment"
                }
    ...
    def _check_human_oversight(self, action_ctx: ActionContext) -> Dict[str, Any]:
        ...
        # Check for specific action types that require human oversight
        if action_ctx.action_type in ["data_export", "data_deletion", "policy_override", "system_shutdown"]:
            return {
                "requires_approval": True,
                "approval_type": "human_review",
                "details": f"Action type '{action_ctx.action_type}' requires human oversight"
            }
        
        # Check for sensitive data operations
        if action_ctx.data_classification in ["personal", "sensitive", "restricted"]:
            if action_ctx.action_type in ["data_processing", "data_sharing", "data_analysis"]:
                return {
                    "requires_approval": True,
                    "approval_type": "human_review",
                    "details": f"Sensitive data operation requires human approval"
                }
        
        # Check for actions affecting multiple users
        if action_ctx.metadata.get("affects_multiple_users", False):
            return {
                "requires_approval": True,
                "approval_type": "human_review",
                "details": "Actions affecting multiple users require human approval"
            }
        
        # Check for emergency override scenarios
        if action_ctx.metadata.get("emergency_override", False):
            return {
                "requires_approval": True,
                "approval_type": "emergency_human_review",
                "details": "Emergency override requires immediate human approval"
            }
