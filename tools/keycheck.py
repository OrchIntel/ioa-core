...
#!/usr/bin/env python3
"""
IOA Module: tools/keycheck.py
Version: v2.5.0
Last-Updated: 2025-09-04
Agents: Cursor assist
Summary: API Key & Ollama Environment Check using ioa doctor --json as source of truth
...
def load_dotenv_if_requested(use_dotenv: bool = False) -> None:
...
def generate_status_report(keys_data, ollama_data, disk_data, output_path):
    ...
    for provider_key, display_name in provider_mapping.items():
        ...
        key_present = "yes" if configured else "no"
    ...
    cli_status = "ok" if ollama_data.get("cli_available", False) else "failed"
    ...
    if models:
        ...
        ollama_table += f"| Models Available | ok | {len(models)} model(s): {model_list} |\n"
    else:
        ollama_table += "| Models Available | failed | No models found |\n"

    disk_gb = disk_data.get("available_gb", 0)
    disk_status = "ok" if disk_gb >= 8 else "failed"
    ...
    report_content = f"""# STATUS REPORT: API Key & Ollama Environment Check
...
def main():
    ...
    parser.add_argument(
        "--output",
        default="docs/ops/status_reports/STATUS_REPORT_KEYCHECK_20250904.md",
