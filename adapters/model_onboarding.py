...
"""
Model Onboarding Module - ONBOARD-001 Complete

Automated model registration and onboarding system with comprehensive manifest
validation, tenant isolation, and trust verification. Provides seamless integration
for new models into the IOA ecosystem with governance compliance.

Key Features:
- JSON schema-based manifest validation
- Multi-tenant model isolation and scoping
...
class OnboardStatus(Enum):
    """Model onboarding status enumeration."""
    PENDING = "pending"
...
class ModelManifest:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'ModelManifest':
        """Create ModelManifest from dictionary."""
        return cls(
...
class OnboardingResult:
    """Result of model onboarding process."""
    model_id: str
...
class ModelOnboardingError(Exception):
    """Base exception for model onboarding operations."""
    pass


class ManifestValidationError(ModelOnboardingError):
    """Raised when manifest validation fails."""
    pass


class TrustVerificationError(ModelOnboardingError):
    """Raised when trust verification fails."""
    pass


class TenantIsolationError(ModelOnboardingError):
...
class ModelOnboarding:
    """
    Model onboarding system with comprehensive validation and tenant isolation.
    
    Manages the complete model registration lifecycle from manifest validation
    ...
    def __init__(self, 
        ...
        """
        Initialize model onboarding system.
        ...
        self._onboarded_models: Dict[str, ModelManifest] = {}
        self._tenant_models: Dict[str, Set[str]] = {}  # tenant_id -> {model_ids}
        ...
        self.manifest_registry_file = self.base_dir / "model_manifest_registry.json"
    ...
    def _load_manifest_schema(self) -> Optional[Dict[str, Any]]:
        """Load model onboarding JSON schema."""
        schema_path = self.base_dir / "config" / "model_onboarding_schema.json"
    ...
    def validate_manifest_schema(self, manifest_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        """
        Validate model manifest against JSON schema.
    ...
    def _manual_schema_validation(self, data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        required_fields = {'model_id', 'adapter_class', 'capabilities', 'tenant_id', 'trust_signature'}
        ...
        if 'model_id' in data:
            if not isinstance(data['model_id'], str) or len(data['model_id']) < 3:
                errors.append("model_id must be string with minimum 3 characters")
    ...
    def verify_trust_signature(self, manifest: ModelManifest) -> bool:
        """
    ...
    def _calculate_manifest_signature(self, manifest: ModelManifest) -> str:
        """Calculate SHA-256 signature for manifest (simplified)."""
        signature_data = f"{manifest.model_id}:{manifest.adapter_class}:{manifest.tenant_id}"
        return hashlib.sha256(signature_data.encode()).hexdigest()
    
    def onboard_model(self, manifest_data: Dict[str, Any]) -> OnboardingResult:
        """
        Complete model onboarding process with validation and registration.
        
        Args:
            manifest_data: Model manifest data
            
        Returns:
            OnboardingResult with status and metrics
        """
        start_time = datetime.now()
        model_id = manifest_data.get('model_id', 'unknown')
    ...
    def _validate_tenant_isolation(self, manifest: ModelManifest) -> bool:
        """Validate tenant isolation requirements."""
        # Check for agent ID conflicts within tenant
        if manifest.model_id in self._onboarded_models:
            existing_manifest = self._onboarded_models[manifest.model_id]
    ...
    def _register_model(self, manifest: ModelManifest) -> None:
        """Register model in internal registry."""
        self._onboarded_models[manifest.model_id] = manifest
        
        # Update tenant tracking
        if manifest.tenant_id not in self._tenant_models:
            self._tenant_models[manifest.tenant_id] = set()
        self._tenant_models[manifest.tenant_id].add(manifest.model_id)
    
    def get_model_status(self, model_id: str) -> Optional[ModelManifest]:
        """Get model manifest by ID."""
        with self._lock:
            return self._onboarded_models.get(model_id)
    
    def list_tenant_models(self, tenant_id: str) -> List[str]:
        """List all models for a specific tenant."""
        with self._lock:
            return list(self._tenant_models.get(tenant_id, set()))
    ...
    def get_onboarding_metrics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            metrics = {
                ...
                "active_models": len(self._onboarded_models),
                "tenant_count": len(self._tenant_models),
    ...
    # PATCH: Cursor-2025-08-19 DISPATCH-GPT-20250819-022 <add PKI functionality for integration wiring>
        """
        Issue a signed model manifest for onboarding.
        ...
        try:
            ...
            self._save_manifest_registry(model_id, registry_entry)
            ...
            get_audit_chain().log("security.model_issued", audit_data)
            
            self.logger.info(f"Manifest issued for model {model_id} with fingerprint {pubkey_fpr}")
...
def create_model_onboarding(base_dir: Optional[Path] = None,
                           validation_level: ValidationLevel = ValidationLevel.STANDARD) -> ModelOnboarding:
    """Create configured ModelOnboarding instance."""
    return ModelOnboarding(base_dir=base_dir, validation_level=validation_level)
