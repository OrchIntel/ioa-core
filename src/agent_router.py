...
# Module: agent_router.py | Version: v2.4.8 | License: Apache-2.0
...
# IOA Module: src/agent_router.py
# Version: v2.5.0
# Last-Updated: 2025-08-19
# Agents: Cursor assist
# Summary: Multi-agent orchestration hub with audit logging
...
class AgentRouter:
    ...
    def register_real_agent(
        ...
        try:
            with self._lock:
                ...
                print(f"[AgentRouter] Registered LLM model: {name} ({agent_id})")
                return True
    ...
    def register_virtual_agent(
        ...
        try:
            with self._lock:
                ...
                print(f"[AgentRouter] Registered virtual agent: {name} ({agent_id})")
                return True
    ...
    def route_task(
        ...
        try:
            with self._lock:
                ...
                return {selected_agent: result.result}
    ...
    def apply_compiled_plan(self, plan: Dict[str, Any]) -> None:
        ...
        """
        try:
            agents: List[str] = list(plan.get("agents", []))
        except Exception:
            agents = []
        if not agents:
            return
