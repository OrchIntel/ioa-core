...
def _run_provider_call(
    ...
    try:
        ...
        completion_text = service.execute(
            prompt, timeout=timeout_ms // 1000, max_tokens=max_tokens
        )
...
def _save_transcript(
    provider_id: str, result: Dict[str, Any], artifacts_dir: Path
) -> None:
...
def _create_final_pitch(agent_results: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    successful_results = {
        k: v for k, v in agent_results.items() if v.get("status") == "passed"
    }
    ...
    for i in range(1, 6):
        ...
        if "openai" in successful_results:
            slide_content += (
                f" - {successful_results['openai'].get('completion_text', '')[:100]}..."
            )
        slides.append(
            {
                "slide_number": i,
...
def run_roundtable_demo(
    ...
    click.echo("=" * 50)
    click.echo(
        "Creating 5-slide pitch for EcoLens (AI for small farms' water optimization)"
    )
    click.echo()
    ...
    try:
        ...
        cumulative_cost = 0.0
            ...
            if cumulative_cost >= max_usd:
                click.echo(
                    f"⏭️ Skipping {agent['name']} (cost cap reached: ${cumulative_cost:.6f})"
                )
            ...
            if result["status"] == "passed":
                click.echo(
                    f"  ✅ {agent['name']}: {result['latency_ms']}ms, ${result['estimated_usd']:.6f}"
                )
