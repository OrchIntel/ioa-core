...
# IOA Module: src/cli/pr_bot_utils.py
# Version: v2.5.0
# Last-Updated: 2025-09-19
# Agents: Cursor assist
# Summary: PR bot comment utilities for CI Gates v1
...
class PRBotCommentGenerator:
    ...
    def generate_comment(self, summary_data: Dict[str, Any], pr_number: int) -> str:
        ...
        comment_lines.append(f"- [summary.json]({summary_data['artifacts_dir']}/summary.json)")
        comment_lines.append(f"- [timeseries.jsonl]({summary_data['artifacts_dir']}/timeseries.jsonl)")
        ...
        # Re-run command
        comment_lines.append(f"**Re-run locally:** `ioa gates doctor --mode {summary_data['mode']} --requests 500`")
    ...
    def generate_status_report(self, summary_data: Dict[str, Any]) -> str:
        ...
        report_lines.append("## Artifacts")
        report_lines.append("")
        report_lines.append(f"All artifacts are stored in: `{summary_data['artifacts_dir']}`")
        report_lines.append("")
        report_lines.append(f"- **Summary:** `{summary_data['artifacts_dir']}/summary.json`")
        report_lines.append(f"- **Timeseries:** `{summary_data['artifacts_dir']}/timeseries.jsonl`")
        report_lines.append("")
    ...
    def save_status_report(self, report: str, filename: str = "status_report.md") -> Path:
