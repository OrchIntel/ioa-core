...
# IOA Module: src/cli/security_utils.py
# Version: v2.5.0
# Last-Updated: 2025-09-19
# Agents: Cursor assist
# Summary: Security utilities for CI Gates v1 - Bandit, TruffleHog, and hygiene checks
...
class SecurityScanner:
    ...
    def run_bandit_scan(self, source_dir: str = "src/", config: Dict[str, Any] = None) -> SecurityScanResult:
    ...
    def run_hygiene_check(self, forbidden_patterns: List[str], config: Dict[str, Any] = None) -> SecurityScanResult:
    ...
    def run_security_suite(self, config: Dict[str, Any]) -> Dict[str, SecurityScanResult]:
                ...
                for issue in result.issues[:10]:  # Limit to first 10
                    if isinstance(issue, dict):
                        if "file" in issue and "pattern" in issue:
                            report_lines.append(f"- {issue['file']} (pattern: {issue['pattern']})")
                        elif "filename" in issue and "test_id" in issue:
                            report_lines.append(f"- {issue['filename']}:{issue['line_number']} - {issue['test_id']}")
                        else:
