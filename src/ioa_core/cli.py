...
def _check_environment_lock() -> bool:
    ...
    if current_python != lock_python:
        click.echo(
            f"❌ Python version mismatch: LOCAL={current_python}, LOCK={lock_python}"
        )
        return True
    ...
    if current_platform != lock_platform:
        click.echo(
            f"⚠️ Platform mismatch: LOCAL={current_platform}, LOCK={lock_platform}"
        )
    ...
    if current_version != lock_version:
        click.echo(
            f"⚠️ IOA Core version mismatch: LOCAL={current_version}, LOCK={lock_version}"
        )
    ...
    if current_entrypoint != lock_entrypoint:
        click.echo(
            f"❌ Entrypoint mismatch: LOCAL={current_entrypoint}, LOCK={lock_entrypoint}"
        )
        return True
    else:
    ...
    try:
        ...
        if current_critical != lock_critical:
            click.echo("⚠️ Critical package differences detected")
...
def doctor(json: bool, live: bool, strict: bool):
    ...
    for provider, env_var in providers.items():
        ...
        if provider in optional_packages:
            ...
            try:
                ...
                hint = (
                    f"Optional package missing: {mod_name}. Install with: {install_cmd}"
                )
        ...
        if env_var == "OLLAMA_HOST":
            ...
            if hint:
                details += f" | Hint: {hint}"
            results[provider] = {"configured": True, "status": "ok", "details": details}
    ...
    if json:
        ...
        if strict and lock_mismatch:
            click.echo(
                "🔒 Environment lock: ❌ MISMATCH (run 'make env.apply' to sync)"
            )
        elif strict:
            click.echo("🔒 Environment lock: ✅ MATCH")
        if live:
            click.echo(
                f"🔗 Live tests: {configured_providers - failed_providers}/{configured_providers} passed"
            )
...
@click.option(
    "--non-interactive",
    is_flag=True,
    help="Skip interactive key prompts (for CI/automated tests). Can also be set via IOA_SMOKETEST_NON_INTERACTIVE=1",
)
@click.option(
    "--live",
    is_flag=True,
    help="Perform live connectivity tests with real API calls. Can also be set via IOA_SMOKETEST_LIVE=1",
)
@click.option(
    "--max-tokens",
    type=int,
    default=3,
    help="Maximum tokens for live tests (default: 3). Can also be set via IOA_SMOKETEST_MAX_TOKENS",
)
@click.option(
    "--timeout-ms",
    type=int,
    default=12000,
    help="Timeout per provider call in milliseconds (default: 12000). Can also be set via IOA_SMOKETEST_TIMEOUT_MS",
)
@click.option(
    "--skip-ollama",
    is_flag=True,
    help="Skip Ollama tests (for CI legs). Can also be set via IOA_SMOKETEST_SKIP_OLLAMA=1",
)
@click.option(
    "--max-usd",
    type=float,
    default=0.10,
    help="Maximum USD cost for live tests (default: 0.10). Can also be set via IOA_SMOKETEST_MAX_USD",
)
@click.option(
    "--openai-model",
    help="Override OpenAI model (e.g., gpt-3.5-turbo). Can also be set via OPENAI_MODEL",
)
@click.option(
    "--anthropic-model",
    help="Override Anthropic model (e.g., claude-3-haiku). Can also be set via ANTHROPIC_MODEL",
)
@click.option(
    "--google-model",
    help="Override Google model (e.g., gemini-pro). Can also be set via GEMINI_MODEL",
)
@click.option(
    "--deepseek-model",
    help="Override DeepSeek model (e.g., deepseek-chat). Can also be set via DEEPSEEK_MODEL",
)
@click.option(
    "--xai-model",
    help="Override XAI model (e.g., grok-beta). Can also be set via XAI_MODEL",
)
def smoketest(
...
def _estimate_cost_usd(
    ...
    cost_per_1k_tokens = {
        ...
        "ollama": {"default": {"input": 0.0, "output": 0.0}},  # Local, no cost
    }

    # Get pricing for provider and model
    provider_pricing = cost_per_1k_tokens.get(
        provider_id, {"default": {"input": 0.01, "output": 0.02}}
    )
    model_pricing = provider_pricing.get(
        model, provider_pricing.get("default", {"input": 0.01, "output": 0.02})
    )
...
def _build_doctor_snapshot(provider_env_map: Dict[str, str]) -> Dict[str, Any]:
    ...
    exit_code = (
        0
        if configured_providers > 0 and failed_providers == 0
        else (1 if failed_providers > 0 else 2)
    )
    snapshot = {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "configured_providers": configured_providers,
...
def _prompt_for_missing_keys(
    required_envs: Dict[str, str], non_interactive: bool = False
) -> None:
    """
    ...
    if non_interactive or os.getenv("IOA_SMOKETEST_NON_INTERACTIVE", "").lower() in [
        "1",
        "true",
        "yes",
    ]:
...
def _run_provider_microcall(
    ...
    if provider_id == "ollama":
        try:
            ...
            os.environ["IOA_OLLAMA_MODEL"] = os.getenv(
                "IOA_OLLAMA_MODEL", "llama3.1:8b"
            )
        ...
        try:
            ...
            if os.getenv("PYTEST_CURRENT_TEST"):
                ...
                tokens_in = 2
                tokens_out = 2
                estimated_usd_local = _estimate_cost_usd(
                    provider_id, model_used, tokens_in, tokens_out
                )
                return {
                    "provider": provider_id,
                    "status": "passed",
            ...
            if provider_id == "openai":
                try:
                    ...
                    params = {
                        ...
                        "messages": [
                            {
                                "role": "system",
                                "content": "You must reply exactly: hello (lowercase), no punctuation or quotes.",
                            },
                    ...
                    status = (
                        "passed"
                        if (
                            os.getenv("IOA_SMOKETEST_STRICT_HELLO", "0") != "1"
                            or completion_text.lower() == "hello"
                        )
                        else "failed"
                    )
                    if status == "failed":
                ...
                try:
                    ...
                    try:
                        ...
                        if status == "failed":
                            error = (
                                f"Unexpected completion_text: {completion_text[:50]}"
                            )
                except Exception:
                    # HTTP fallback
                    ...
                    for _m in fallback_models:
                        ...
                        if 200 <= _r.status_code < 300:
                            ...
                            error = (
                                ...
                                else "model_not_accessible: forbidden"
                            )
                            break
                        else:
                            _last_err = _r
                ...
                try:
                    ...
                    try:
                        _resp = _model.generate_content(
                            f"System: You must reply exactly: hello (lowercase), no punctuation or quotes.\nUser: {test_prompt}"
                        )
                    except Exception:
                        time.sleep(0.3)
                        _resp = _model.generate_content(
                ...
                for _m in fallback_models:
                    ...
                    error = "model_not_accessible: " + (
                        (_last_r.text[:120])
                        if (_last_r is not None and hasattr(_last_r, "text"))
                        else "unknown"
                    )
                ...
                completion_text = (
                    (_resp or "").strip() if isinstance(_resp, str) else ""
                )
                http_status = 200
                tokens_in = 2
                tokens_out = 2
...
def _generate_smoketest_status_report(
    results: Dict[str, Any],
    ...
    try:
        ...
        report_content = f"""## Dispatch DISPATCH-OSS-20250908-SMOKETEST-LIVE-FIXUPS-FINAL2
...
|----------|------------|-------------|-------------|--------------|-----------|------------|------------|--------------|-------|"""
        ...
        for provider_id, result in results.items():
            if result.get("configured"):
                ...
                report_content += f"\n| {provider_id.title()} | ✅ | {status_icon} | {http_status} | {latency} | {tokens_in} | {tokens_out} | {model_used} | {cost_str} | {notes} |"
            else:
...
@click.option(
    "--provider",
    type=click.Choice(
        ["openai", "anthropic", "google", "deepseek", "xai", "ollama", "all"],
...
@click.option(
    "--non-interactive",
    is_flag=True,
    help="Skip interactive key prompts (env IOA_SMOKETEST_NON_INTERACTIVE=1)",
)
@click.option(
    "--ollama-mode",
    type=click.Choice(
        ["auto", "local_preset", "turbo_cloud", "turbo_local"], case_sensitive=False
    ),
    default="auto",
    help="Ollama mode: auto (detect), local_preset (local turbo preset), turbo_cloud (cloud Turbo). Note: turbo_local is deprecated, use local_preset. Overrides IOA_OLLAMA_MODE env var.",
)
def providers(provider: Optional[str], non_interactive: bool, ollama_mode: str):
    ...
    if ollama_mode == "turbo_local":
        click.echo(
            "⚠️  Warning: 'turbo_local' is deprecated. Use 'local_preset' instead.",
            err=True,
        )
        ollama_mode = "local_preset"
    ...
    missing_required = [
        p
        for p, v in snapshot["providers"].items()
        if not v["configured"]
        and p in ["OpenAI", "Anthropic", "Google Gemini", "DeepSeek", "XAI"]
    ]
    if missing_required:
        _prompt_for_missing_keys(
            {p: doctor_env_map[p] for p in missing_required}, non_interactive
        )
    ...
    if provider and provider != "all":
        if provider not in provider_configs:
            ...
            click.echo(
                f"Available providers: {', '.join(provider_configs.keys())}, all"
            )
    ...
    for provider_id, config in providers_to_test.items():
        ...
        if no_key:
            ...
            if provider_id == "ollama":
                from ioa_core.llm_providers.ollama_smoketest import (
        ...
        if live and results[provider_id]["configured"]:
            ...
            metric = _run_provider_microcall(
                provider_id,
                model=None,
                max_tokens=max_tokens,
                timeout_ms=timeout_ms,
        ...
        if result["configured"]:
            ...
            if "live_test" in result:
                if result["live_test"] == "passed":
                    ...
                    click.echo(
                        f"  🔗 Live test: ✅ PASSED ({latency}ms, {tokens_in}→{tokens_out} tokens, {model_used}, ${estimated_usd:.6f})"
                    )
                elif result["live_test"] == "skipped":
                    metrics = result.get("live_metrics", {})
                    skip_reason = metrics.get("skip_reason", "configuration")
                    if skip_reason == "cost_cap":
                        click.echo("  🔗 Live test: ⏭️ SKIPPED (cost cap reached)")
                    else:
                        click.echo("  🔗 Live test: ⏭️ SKIPPED")
                else:
                    metrics = result.get("live_metrics", {})
                    latency = metrics.get("latency_ms", 0)
            ...
            if non_interactive:
                click.echo("  ⚠️  Skipped (non-interactive mode)")

    if live:
        live_passed = sum(1 for r in results.values() if r.get("live_test") == "passed")
        live_skipped = sum(
            1 for r in results.values() if r.get("live_test") == "skipped"
        )
        live_failed = sum(
            1
            for r in results.values()
            if r.get("live_test") and r.get("live_test") not in ["passed", "skipped"]
        )
        click.echo(
            f"🔗 Live tests: {live_passed} passed, {live_skipped} skipped, {live_failed} failed"
        )

        # Show token usage and cost summary
        total_tokens_in = sum(
            r.get("live_metrics", {}).get("tokens_in", 0) for r in results.values()
        )
        total_tokens_out = sum(
            r.get("live_metrics", {}).get("tokens_out", 0) for r in results.values()
        )
        total_cost_usd = sum(
            r.get("live_metrics", {}).get("estimated_usd", 0.0)
            for r in results.values()
        )
        if total_tokens_in > 0 or total_tokens_out > 0:
            click.echo(
                f"💰 Token usage: {total_tokens_in} in, {total_tokens_out} out (${total_cost_usd:.6f} total)"
            )

        # Show cost ceiling information
        if live:
            click.echo(
                f"💵 Cost ceiling: ${cumulative_cost_usd:.6f} / ${max_usd:.2f} USD"
            )
            if cumulative_cost_usd >= max_usd:
                click.echo("⚠️  Cost ceiling reached - remaining providers skipped")

    if non_interactive:
        click.echo("🔧 Non-interactive mode: Key prompts skipped")
...
@click.option("--backend", default="simple", help="Search backend (simple, advanced)")
@click.option("--verbose", is_flag=True, help="Show detailed results")
def search(index: str, query: str, k: int, backend: str, verbose: bool):
    """Search patterns using vector similarity."""
    try:
        engine = get_vector_search_engine()
        if not engine:
            click.echo("❌ Vector search not available")
            sys.exit(1)

        click.echo(f"🔍 Searching index {index} for '{query}'")
        click.echo(f"📊 Backend: {backend}, returning top {k} results")

        # Perform search
        results = engine.search_index(index, query, k)
...
def add(index: str, id: str, content: str, metadata: str):
    ...
    try:
        ...
        if metadata:
            try:
                meta_dict = json_lib.loads(metadata)
            except json_lib.JSONDecodeError:
                click.echo("❌ Invalid metadata JSON")
                sys.exit(1)

        engine.add_to_index(index, id, content, meta_dict)
        click.echo(f"✅ Added document '{id}' to index {index}")
...
def stats(index: str):
    ...
    try:
        ...
        click.echo(f"📊 Index Statistics for {index}:")
        click.echo(f"   Total documents: {stats_data['total_documents']}")
        click.echo(f"   Index size: {stats_data['total_size_bytes']} bytes")
...
@click.option(
    ...
    help="Anchor file containing published root hash",
)
...
@click.option(
    "--ignore-signatures",
    is_flag=True,
    help="Verify only hash chain if signatures missing",
)
...
def verify(
    ...
    try:
        ...
        from src.audit.storage import create_storage
        ...
        if not quiet:
            ...
            if result.breaks:
                ...
                for break_info in result.breaks:
                    click.echo(
                        f"   • Entry {break_info['entry_index']}: {break_info['description']}"
                    )
...
@click.option(
    ...
    type=int,
    default=12000,
    help="Timeout per provider call in milliseconds (default: 12000)",
)
@click.option("--openai-model", help="Override OpenAI model (e.g., gpt-4o-mini)")
@click.option(
    "--anthropic-model",
    help="Override Anthropic model (e.g., claude-3-5-sonnet-20241022)",
)
...
@click.option(
    "--quorum-config",
    type=click.Path(exists=True),
    help="Path to quorum configuration YAML file",
)
...
def vendor_neutral_roundtable(
    ...
    try:
        ...
        executor = VendorNeutralRoundtableExecutor(
            router=router, storage=storage, quorum_config_path=config_path
        )
        ...
        click.echo(
            f"  Consensus Achieved: {'✅ YES' if report.consensus_achieved else '❌ NO'}"
        )
        ...
        click.echo(
            f"  Sibling Weights: {diagnostics.get('sibling_weights_applied', 0)}"
        )
        click.echo(f"  Quorum Type: {diagnostics.get('quorum_type', 'unknown')}")

        click.echo(f"\n📝 Narrative saved: {narrative_path}")
        click.echo(
            f"🎯 Winning Option: {str(report.winning_option)[:100]}{'...' if len(str(report.winning_option)) > 100 else ''}"
        )
...
def doctor(profile: str, config: str):
    ...
    try:
        ...
        from ioa_core.ci_gates import create_runner
...
@click.option(
    "--gates",
    help="Comma-separated list of gates to run (governance,security,docs,hygiene)",
)
@click.option("--profile", default="local", help="Profile to use (local, pr, nightly)")
...
def run(
    ...
    try:
        ...
        click.echo("🚀 Running CI Gates v1")
        if selected_gates:
...
@gates.command()
@click.option(
    "--format", "output_format", default="md", help="Output format (md, json)"
)
