...
# IOA Module: src/cli/ci_gates.py
# Version: v2.5.0
# Last-Updated: 2025-09-19
# Agents: Cursor assist
# Summary: CI Gates v1 implementation for governance, security, docs, and hygiene validation
    ...
    def get_profile(self, profile: str) -> Dict[str, Any]:
        ...
        if "sustainability" in profile_config:
            if isinstance(profile_config["sustainability"], bool):
                base_config["sustainability"]["enabled"] = profile_config["sustainability"]
            else:
...
class CIGatesRunner:
    ...
    def __init__(self, config: CIGatesConfig, profile: str = "local", artifacts_dir: Optional[Path] = None):
        self.config = config
    ...
    def run_gates(self, selected_gates: Optional[List[str]] = None) -> GatesSummary:
        ...
        click.echo(f"🚀 Running CI Gates v1 (profile: {self.profile}, mode: {self.profile_config['mode']})")
        click.echo("=" * 60)
    ...
    def _run_governance_gate(self) -> GateResult:
        ...
        try:
            ...
            harness_requests = self.profile_config.get("harness", {}).get("requests", 1000)
            laws = self.profile_config.get("laws", [1,2,3,4,5,6,7])
            
            # Run ethics
            if self.profile_config.get("ethics", {}).get("mode") != "disabled":
                click.echo("   Running ethics validation...")
                ethics_sample = min(harness_requests, 200)  # Cap ethics sample size
                ethics_result = self._run_command([
                    "python", "-m", "ioa_core.cli", "ethics", "run",
                    "--sample", str(ethics_sample)
                ])
                details["ethics"] = "completed" if ethics_result.returncode == 0 else "failed"
                details["ethics_exit_code"] = ethics_result.returncode
                details["ethics_sample_size"] = ethics_sample
                
                if ethics_result.returncode != 0:
                    details["ethics_error"] = ethics_result.stderr
                    details["ethics_stdout"] = ethics_result.stdout
            
            # Run sustainability (check if enabled for this profile)
            sustainability_enabled = self.profile_config.get("sustainability", {}).get("enabled", True)
            if sustainability_enabled and self.profile_config.get("sustainability", {}).get("mode") != "disabled":
                click.echo("   Running sustainability validation...")
                sustainability_sample = min(harness_requests, 100)  # Cap sustainability sample size
                sustainability_result = self._run_command([
                    "python", "-m", "ioa_core.cli", "sustainability", "run",
                    "--sample", str(sustainability_sample)
                ])
                details["sustainability"] = "completed" if sustainability_result.returncode == 0 else "failed"
                details["sustainability_exit_code"] = sustainability_result.returncode
                details["sustainability_sample_size"] = sustainability_sample
                
                if sustainability_result.returncode != 0:
                    details["sustainability_error"] = sustainability_result.stderr
                    details["sustainability_stdout"] = sustainability_result.stdout
            else:
                click.echo("   Skipping sustainability validation (disabled for this profile)")
                details["sustainability"] = "skipped"
                details["sustainability_reason"] = "disabled for profile"
            
            # Run harness
            click.echo("   Running governance harness...")
            harness_result = self._run_command([
                "python", "-m", "ioa_core.cli", "harness", "run",
                "--requests", str(harness_requests),
                "--laws", ",".join(map(str, laws))
            ])
            details["harness"] = "completed" if harness_result.returncode == 0 else "failed"
            details["harness_exit_code"] = harness_result.returncode
            details["harness_requests"] = harness_requests
            details["harness_laws"] = laws
            
            if harness_result.returncode != 0:
                details["harness_error"] = harness_result.stderr
                details["harness_stdout"] = harness_result.stdout
            
            # Parse harness output for additional details
            if harness_result.returncode == 0 and harness_result.stdout:
                stdout_lines = harness_result.stdout.split('\n')
                for line in stdout_lines:
                    if "Total Requests:" in line:
                        details["harness_total_requests"] = line.split(":")[-1].strip()
                    elif "P95 Latency:" in line:
                        details["harness_p95_latency"] = line.split(":")[-1].strip()
                    elif "Assurance Score:" in line:
                        details["harness_assurance_score"] = line.split(":")[-1].strip()
                    elif "Ready Status:" in line:
                        details["harness_ready_status"] = line.split(":")[-1].strip()
            
            # Determine overall status
            ethics_passed = (
                details.get("ethics") == "completed" or 
                self.profile_config.get("ethics", {}).get("mode") == "disabled"
            )
            sustainability_passed = (
                details.get("sustainability") == "completed" or 
                details.get("sustainability") == "skipped" or
                self.profile_config.get("sustainability", {}).get("mode") == "disabled"
            )
            harness_passed = details.get("harness") == "completed"
    ...
    def _run_security_gate(self) -> GateResult:
        ...
        try:
            ...
            # Determine overall status
            status = GateStatus.PASS if all_passed else GateStatus.FAIL
            message = f"Security scan passed ({total_issues} issues found)" if all_passed else f"Security issues found ({total_issues} total)"
    ...
    def _run_docs_gate(self) -> GateResult:
        ...
        try:
            # Run CLI validator
            cli_validator_config = self.profile_config.get("docs", {}).get("cli_validator", {})
            ...
            if cli_strict:
                ...
                cli_result = self._run_command([
                    "python", "tools/doc_cmd_validator/extract_and_run.py",
                    "--repo-root", ".",
                ...
                details["cli_validator"] = "completed" if cli_result.returncode == 0 else "failed"
                ...
                if cli_json_file.exists():
                    ...
                    details["cli_commands_tested"] = cli_data.get("summary", {}).get("total_commands", 0)
                    details["cli_commands_passed"] = cli_data.get("summary", {}).get("successful_commands", 0)
                    details["cli_commands_failed"] = cli_data.get("summary", {}).get("failed_commands", 0)
            ...
            # Run MkDocs build
            link_check_config = self.profile_config.get("docs", {}).get("link_check", {})
            ...
            if link_check_enabled:
                ...
                details["mkdocs"] = "completed" if mkdocs_result.returncode == 0 else "failed"
            ...
            cli_passed = (
                details.get("cli_validator") == "completed" or 
                not cli_strict
            )
            mkdocs_passed = (
                details.get("mkdocs") == "completed" or 
                details.get("mkdocs") == "skipped" or
                not link_check_enabled
            )
    ...
    def _save_summary(self, summary: GatesSummary):
        ...
        try:
            ...
            # Generate PR comment
            pr_comment = pr_bot.generate_comment(asdict(summary), 0)  # PR number 0 for local runs
    ...
    def _display_summary(self, summary: GatesSummary):
        ...
        click.echo(f"Summary: {summary.artifacts_dir}/summary.json")
...
def create_runner(profile: str = "local", config_path: str = ".ioa/ci-gates.yml") -> CIGatesRunner:
