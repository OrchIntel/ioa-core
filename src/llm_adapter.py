...
# Module: llm_adapter.py | Version: v2.5.0 | License: Apache-2.0
...
from ioa_core.llm_manager import LLMManager, LLMProviderError
...
class OpenAIService(LLMService):
    ...
    def __init__(self, model: str = None, api_key: Optional[str] = None, provider: str = "openai") -> None:
        """
        ...
        try:
            ...
            # Preserve strict explicit-key precedence for backward compatibility.
            if api_key is not None:
                self.api_key = api_key
            else:
                resolved_api_key = llm_manager.resolve_api_key(provider, explicit_key=None)
                if resolved_api_key:
            ...
            if provider in llm_manager.list_providers():
                ...
                # Keep legacy precedence expected by compatibility tests:
                # explicit model > env model > manager model (only when key not explicitly passed) > default.
                if model:
                    self.model = model
                elif env_model:
                    self.model = env_model
                elif api_key is None and provider_config.get("model"):
