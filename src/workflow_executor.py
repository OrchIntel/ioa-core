...
"""
IOA Module: src/workflow_executor.py
Version: v2.5.0
Last-Updated: 2025-08-18
Agents: Cursor assist
Summary: Minimal production-grade WorkflowExecutor that compiles DSL → execution, runs via AgentRouter+Roundtable, and saves artifacts.
...
- Save `FinalReport` and logs into a runtime-safe artifacts directory or the DSL-specified `artifacts.output_dir`.
...
from src.roundtable_executor import RoundtableExecutor, FinalReport
from src.storage_adapter import JSONStorageService
...
def _runtime_artifacts_root() -> Path:
    """Resolve the runtime artifacts root outside the tracked repo by default."""
    override = os.getenv("IOA_RUNTIME_ARTIFACTS_ROOT")
    if override:
        return Path(override)
    return Path(tempfile.gettempdir()) / "ioa-runtime" / "ioa-core"


def _resolve_output_dir(raw_output_dir: str) -> str:
    """Redirect repo-local reports paths to the runtime artifacts root."""
    expanded = raw_output_dir.replace("${STAMP}", _stamp_now())
    out_path = Path(expanded)
    if out_path.is_absolute():
        return str(out_path)
    if expanded == "reports" or expanded.startswith("reports/"):
        return str(_runtime_artifacts_root() / expanded)
    return str(out_path)
...
class WorkflowExecutor:
    ...
    def parse_yaml(self, path: str) -> WorkflowSpec:
        ...
        def _repair_legacy_agents_block(text: str) -> str:
            """Repair legacy DSL where agent list items were placed after orchestration fields."""
            ...
            for line in lines:
                ...
                if stripped.startswith("orchestration:"):
                    in_orch = True
                    orch_indent = indent
                    inserted_agents = False
                    out.append(line)
                ...
                if in_orch and stripped and indent <= orch_indent and not stripped.startswith("#"):
                    in_orch = False

                if in_orch and indent == orch_indent + 2 and stripped.startswith("- "):
                    if not inserted_agents:
                        out.append(" " * orch_indent + "agents:")
        ...
        try:
            ...
            data = yaml.safe_load(_repair_legacy_agents_block(raw_yaml)) or {}
    ...
    def compile(self, spec: WorkflowSpec) -> CompiledPlan:
        ...
        out_dir = _resolve_output_dir(spec.artifacts.output_dir)
        return CompiledPlan(
